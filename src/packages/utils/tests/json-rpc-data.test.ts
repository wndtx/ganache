...
import {Data, Quantity} from "..";

describe("json-rpc-data", () => {
  const inputOf32Bytes = "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef";
  const validValues = [ "0x", "0x1", "0x1234", Buffer.from([]), Buffer.from([0x12,0x34]), inputOf32Bytes, undefined, null ];
  ...
  function getExpectedString(value: Buffer|string, bytelength?: number) {
    ...
    const length = bytelength ?? Math.floor(expected.length / 2) + (expected.length & 1);
    /*
    ...
    */
    const padCharCount = (length - expected.length / 2) * 2; // (desired byte count - actual byte count) * 2 characters per byte
    if (padCharCount > 0) {
      expected = "0".repeat(padCharCount) + expected;
    } else {
      expected = expected.slice(0, length * 2);
    }
    return "0x" + expected;
  }
  ...
  describe("quantity", () => {
    it("should work", () => {
      const s = Quantity.from(undefined, true).toBigInt();
  ...
  describe("toString()", () => {
    it("should work with uneven length", () => {
      const d= new Data("0x123");
      const s = d.toString();
      assert.equal(s, "0x0123");
    });
  ...
  describe("toBuffer()", () => {
    ...
    it("should create a buffer with a smaller bytelength", () => {
      const expected = Buffer.from([0x01, 0x23]);
      const d = new Data("0x0123456789abcdef", 2);
      const b = d.toBuffer();
    ...
    // todo: this should be addressed in rewrite of json-rpc-data https://github.com/trufflesuite/ganache/labels/json-rpc%20refactor
    it("should create a buffer with a larger bytelength", () => {
